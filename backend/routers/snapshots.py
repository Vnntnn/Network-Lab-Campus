...
async def create_snapshot_record(db: AsyncSession, pod_id: int, label: str = "manual") -> Snapshot:
...
async def capture_snapshot_background(pod_id: int, label: str = "pre-push") -> None:
    async with AsyncSessionLocal() as db:
        try:
            await create_snapshot_record(db=db, pod_id=pod_id, label=label)
        except Exception:
            await db.rollback()


@router.get("/pod/{pod_id}", response_model=list[SnapshotRead])
async def list_snapshots(pod_id: int, db: AsyncSession = Depends(get_db)):
...
async def capture_snapshot(pod_id: int, label: str = "manual", db: AsyncSession = Depends(get_db)):
    return await create_snapshot_record(db=db, pod_id=pod_id, label=label)


@router.post("/{snap_id}/rollback")
async def rollback_snapshot(snap_id: int, db: AsyncSession = Depends(get_db)):
    snap = await db.get(Snapshot, snap_id)
    if not snap:
        raise HTTPException(status_code=404, detail="Snapshot not found")

    pod = await db.get(LabPod, snap.pod_id)
    if not pod:
        raise HTTPException(status_code=404, detail="Pod not found")
...
async def delete_snapshot(snap_id: int, db: AsyncSession = Depends(get_db)):
    await db.execute(delete(Snapshot).where(Snapshot.id == snap_id))
    await db.commit()
